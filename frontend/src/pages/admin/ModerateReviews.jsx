...
import { getAllReviewsAdmin, updateReviewStatusAdmin, deleteReviewAdmin } from '../../api/adminApi';
...
export default function ModerateReviews() {
  ...
  const filtered = tab === 'All' ? reviews : reviews.filter(r => r.status === tab.toLowerCase());

  const counts = TABS.reduce((acc, t) => {
    acc[t] = t === 'All' ? reviews.length : reviews.filter(r => r.status === t.toLowerCase()).length;
    return acc;
  }, {});
  ...
  const handleToggleVisibility = async (review) => {
    ...
    setBusyId(review.id);
    try {
      await updateReviewStatusAdmin(review.id, nextStatus);
      setReviews(prev => prev.map(r => r.id === review.id ? { ...r, status: nextStatus } : r));
      showToast(nextStatus === 'hidden' ? 'Review hidden.' : 'Review made visible.');
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteReviewAdmin(deleting.id);
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="text-2xl font-extrabold text-gray-800">Moderate Reviews</h1>
        <p className="text-gray-400 text-sm mt-1">Hide or remove reviews that violate platform guidelines</p>
      </motion.div>
        ...
        {TABS.map(t => {
          ...
          return (
            ...
            >
              {isActive && (
                <motion.span
                  layoutId="admin-review-tab-pill"
        ...
        {deleting && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-lg font-bold text-gray-800 text-center mb-1">Delete Review</h3>
              <p className="text-sm text-gray-500 text-center mb-5">
                This permanently removes the review by <strong>{deleting.user_name}</strong>.
              </p>
