              ...
              <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-4">
                <AnimatePresence>
                  {pending.map(item => {
                    ...
                    return (
                      <motion.div key={item.product_id} layout variants={listItem} exit="exit" className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-xl bg-orange-50 flex items-center justify-center overflow-hidden flex-shrink-0">
                              {item.product_image ? (
                                <img src={`${import.meta.env.VITE_API_URL}${item.product_image}`} alt={item.product_name} className="w-full h-full object-contain p-1" />
                              ) : (
                        ...
                        <AnimatePresence mode="wait">
                          {isDone ? (
                            ...
                            >
                              ...
                              <motion.button
                                whileHover={{ scale: form.loading ? 1 : 1.03 }}
                                whileTap={{ scale: form.loading ? 1 : 0.97 }}
                                type="submit"
          ...
          <section>
            ...
            {reviews.length === 0 ? (
              ...
              <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-3">
                {reviews.map(review => (
                  <motion.div key={review.id} variants={listItem} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-xl bg-orange-50 flex items-center justify-center overflow-hidden flex-shrink-0">
                        {review.product_image ? (
                          <img src={`${import.meta.env.VITE_API_URL}${review.product_image}`} alt={review.product_name} className="w-full h-full object-contain p-1" />
                        ) : (
                          <span className="text-xl">🍬</span>
                        )}
                      </div>
