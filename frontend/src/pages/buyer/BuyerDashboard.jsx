...
function StatCard({ icon, value, label, color, index = 0 }) {
  return (
    <motion.div
      custom={index}
      variants={{
  ...
  return (
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard index={0} icon="📦" value={orders.length}   label="Total Orders"    color="bg-orange-100" />
          <StatCard index={1} icon="💜" value={wishlist.length}  label="Wishlist Items"  color="bg-purple-100" />
          <StatCard index={2} icon="⭐" value={reviews.length}   label="Reviews Written" color="bg-yellow-100" />
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="px-6 py-12 text-center">
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="inline-block mt-4">
              <Link to="/products" className="bg-[#F4A261] text-white text-sm px-5 py-2 rounded-full font-semibold hover:bg-[#E76F51] transition-colors inline-block">
                Browse Products
              </Link>
            </motion.div>
          </div>
        ) : (
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="divide-y divide-gray-100">
            {orders.slice(0, 3).map(order => (
              <motion.div key={order.id} variants={listItem} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
                <div>
      ...
      <div>
        <Reveal custom={1} className="flex items-center justify-between mb-4">
        ...
        {loading ? (
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {products.map(p => (
              <motion.div key={p.id} variants={listItem} whileHover={{ y: -4 }} transition={{ type: 'spring', stiffness: 300, damping: 22 }}>
                <Link to={`/products/${p.id}`}
                  className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-200 group block"
                >
                  <div className="relative h-36 bg-orange-50 flex items-center justify-center overflow-hidden">
                    {p.image_url ? (
                      <FitImage
